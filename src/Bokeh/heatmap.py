...
def drawOnFigure(fig, source, heatdata, nodes, palette=hexPaletteToTuplePalette(Inferno256)):
    ...
    populateImagesSourceWithData(image_data, nodes, heatdata, template_base, brush)
...
def populateImagesSourceWithData(image_data, nodes, data, template_base, brush):
    ...
    for node in data:
        ...
        for date in data[node]:
            ...
            for elem in data[node][date]:
                ...
                applyBrush(image_data[image_path], brush, brush_offset, elem[1])


# Change heatmap data from float32 to [uint8]
def reformatHeatmap(array, palette, alpha=223):
...
def applyBrush(base, brush, offset, multiplier):
    # Shift offset by half brush so it points to the center
    brush_half = (constants.HEATMAP_BRUSH_SIZE - 1) / 2
    offset = (offset[0] - brush_half, offset[1] - brush_half)

    # Append brush to image
    for x in range(constants.HEATMAP_BRUSH_SIZE):
        for y in range(constants.HEATMAP_BRUSH_SIZE):
